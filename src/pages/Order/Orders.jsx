  ...
  return (
    <Page>
      <Page.Header 
        ...
        renderBadge={() => `${filteredOrders.length} order${filteredOrders.length !== 1 ? 's' : ''}`}
        renderActions={() => (
          <button
      ...
      <Page.Filters show={showFilters} onClear={handleClearAllFilters} hasActiveFilters={hasActiveFilters()}>
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-3 lg:gap-4 mb-3 lg:mb-4">
          <div>
            <label className="block text-xs lg:text-sm font-semibold text-gray-700 mb-2">Search Orders</label>
            <input
              type="text"
              value={searchText}
