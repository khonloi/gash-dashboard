...
const OrdersByMonth = ({ user }) => {
    ...
    const fetchStats = useCallback(async () => {
        ...
        try {
            setLoading(true);
            const response = await Api.statistics.getOrderStatistics({ period: 'month' });
            setStats(prev => ({
                ...prev,
                ...response,
                ordersPerMonth: response.ordersPerMonth || prev.ordersPerMonth || [],
    ...
    const ordersTrendData = useMemo(() => ({
        labels: Array.isArray(stats.ordersPerMonth) ? stats.ordersPerMonth.map(item => item._id || 'Unknown').slice(-12) : [],
        datasets: [
            {
                label: 'Orders per Month',
                data: Array.isArray(stats.ordersPerMonth) ? stats.ordersPerMonth.map(item => item.count || 0).slice(-12) : [],
                borderColor: '#3b82f6',
        ...
        return (
            <div className="rounded-xl border p-6" style={{ borderColor: 'rgb(217 119 6)', boxShadow: '0 25px 70px rgba(168, 101, 35, 0.3), 0 15px 40px rgba(233, 163, 25, 0.25), 0 5px 15px rgba(168, 101, 35, 0.2)' }} role="status" aria-live="polite">
                <Loading
                    type="page"
                    size="medium"
                    message="Loading Order Statistics by Month..."
                />
            </div>
        );
    }
        ...
        <div className="space-y-6">
            ...
            <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-3 lg:gap-4 mb-4 lg:mb-6 pt-2 lg:pt-3 pb-2 lg:pb-3">
                <div className="flex-1 min-w-0">
                    <h1 className="text-xl sm:text-2xl lg:text-3xl font-bold text-gray-900 mb-1 lg:mb-2 leading-tight">Orders by Month</h1>
                    <p className="text-gray-600 text-sm sm:text-base lg:text-lg">Monthly order performance overview</p>
                </div>
            </div>
                ...
                <div className="p-3 sm:p-4 lg:p-6">
                    <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
                        <div className="bg-gradient-to-br from-gray-50 via-white to-gray-100 rounded-xl p-4 lg:p-6 shadow-inner border border-gray-200">
                            <h3 className="text-base lg:text-lg font-semibold text-gray-900 mb-4">Monthly Orders Trend</h3>
                            <div className="h-64">
